...
class MimeAnalyzer implements LoggerAwareInterface
{
    ...
    /**
     * Get the magic number for GLB files
     * @param string $filename
     * @return string|false
     */
    private function getGlbMagicNumber($filename) {
        $handle = fopen($filename, "rb");
        if ($handle) {
            $magicNumber = fread($handle, 4);
            fclose($handle);
            return bin2hex($magicNumber);
        }
        return false;
    }
	...
	private function doGuessMimeType(string $file)
	{
		...
		// 3D start
		 ...
		 // Check for 3D GLB files
		 $magicNumber = $this->getGlbMagicNumber($file);
		 if ($magicNumber === "676c5446") {
			 $this->logger->info(__METHOD__ . ": Valid GLB file");
